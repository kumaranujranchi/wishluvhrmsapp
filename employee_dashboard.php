...
// Calculate working days for reference (not used in avg calculation anymore)
$days_in_month = cal_days_in_month(CAL_GREGORIAN, $current_month, $current_year);

// Calculate Average Hours based on PRESENT DAYS (not working days)
// This gives: Total Hours ÷ Days Actually Present
...
$apk_info_stmt = $conn->prepare("SELECT setting_key, setting_value FROM system_settings WHERE setting_key IN ('latest_apk_url', 'latest_apk_version')");
$apk_info_stmt->execute();
$apk_settings = $apk_info_stmt->fetchAll(PDO::FETCH_KEY_PAIR);
$latest_apk_url = $apk_settings['latest_apk_url'] ?? '';
$latest_apk_version = $apk_settings['latest_apk_version'] ?? '';
?>
